...
export function ExceliaContactStrip() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmed) return;
    const subject = encodeURIComponent("Excelia: stay in touch");
      ...
      >
        <div className="min-w-0 lg:max-w-none">
          ...
          <p className="mt-2 font-[family-name:var(--font-barlow)] text-[13px] leading-relaxed text-white/85 sm:mt-3 sm:text-sm">
            Write to Excelia about nutmeg shell mulch, stock, and how we route
            shells from Grenadian growers back into soil. We answer gardeners,
            shops, and teams who want waste out of landfill and into beds.
          </p>
          ...
          >
            <label htmlFor="excelia-footer-email" className="sr-only">
              Email address
            </label>
            <input
              id="excelia-footer-email"
              name="email"
              type="email"
              autoComplete="email"
...
export function ExceliaFullFooter() {
  return (
    <footer className="relative isolate z-0">
      <ExceliaContactStrip />
      <ExceliaFooterExplore />
    </footer>
  );
}
